...
class DeciWatch(nn.Module):
    ...
    def forward(self, sequence):
        ...
        self.encoder_mask = encoder_mask.unsqueeze(0).repeat(B, 1).to(self.device)
        self.decoder_mask = decoder_mask.unsqueeze(0).repeat(B, 1).to(self.device)

        self.encoder_pos_embed = self.pos_embed(B, L).to(self.device)
        self.decoder_pos_embed = self.encoder_pos_embed.clone().to(self.device)
...
if __name__ == "__main__":
    ...
    sequence = torch.ones(batch_size, num_frames, keypoints_numel).to(device)
    
    model = DeciWatch(keypoints_numel, 4, 128, 128, enc_layers=5, dec_layers=5, device=device, pre_norm=False).to(device)
